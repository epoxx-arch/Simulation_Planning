...
%% scenery side rear 1 and front vehicle
    % 只有后车 和前车
JerkWeight = 60;
VMaxWeight = 40;
TimeWeight = 60;
...
for ti = 3 : 8 % time i is 3 4 5 6 7 8s
    ...
    % S gap of the window
    window_gap = safe_front - safe_side_rear;
    if window_gap > kEgoLength %
        window_gap_start = safe_side_rear;
        window_gap_end = safe_front;
        si = window_gap_start;
        while si < window_gap_end
            v_max = min(velocity_front + sqrt(2*(prediction_position_front - si)*CrashDeceToFront), ...
                VLimitMax);
